...
> **角色**: 🗺️ **计划与路线图** - 了解"P0/P1/P2 阶段目标"和"下一步做什么"
>
> **何时阅读**:
> - 需要了解下一阶段优先级
> - 规划新任务时对照阶段目标
> - 理解为什么某些功能不在当前范围
>
> **与 CURRENT_TRUTH 关系**: `CURRENT_TRUTH.md` 讲"今天已实现什么";本文档讲"接下来计划做什么"
>
> **边界说明**: 本文档为计划性文档,非实现真值;实现真值在 `runtime/`,验收真值在 `tests/`
>
> **用途**: 给 proposal repo 一个**统一阅读入口**,把"当前真值、下一阶段计划、明确边界"放在一页里。
>
> **注意**: 这个 repo 已在 2026-03-22 升级成**单仓分层 monorepo**:`docs/` 是阅读入口,`runtime/` 是 orchestration 实现真值,`tests/` 是验收真值。后续计划与执行口径应优先在本仓收口。
---
...
1. **gstack-style planning default** ✅
   ...
   - **状态**: 已实现 (`runtime/orchestrator/planning_default.py`)
2. **continuation contract v1** ✅
   - 每个任务 closeout 至少带：`summary`、`decision`、`stopped_because`、`next_step`、`next_owner`、`dispatch_readiness`。
   - **状态**: 已实现 (`runtime/orchestrator/closeout_generator.py`, `runtime/orchestrator/continuation_backends.py`)
3. **coding issue lane baseline** ✅
   - 冻结 `issue_lane_v1` schema：`IssueInput` / `PlanningOutput` / `ExecutionOutput` / `CloseoutOutput` / `IssueLaneContract`
   - 输入支持 GitHub issue URL / 标准化 payload
   - 输出覆盖 patch artifact / PR description / closeout summary
   - 最小链路打通：issue input -> planning -> execution -> closeout
   - **状态**: 已实现 (`runtime/orchestrator/issue_lane_schemas.py`)
   - **测试**: 12 个测试全部通过 (`runtime/tests/orchestrator/test_issue_lane_schemas.py`)
