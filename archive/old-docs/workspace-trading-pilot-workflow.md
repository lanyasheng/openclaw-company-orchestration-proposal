...
**2026-03-20 补充真值**：本 workflow 现在不再只有冻结稿，已经补出一份可直接喂给 minimal scheduler 的 runtime definition：
- runtime workflow：`examples/workflows/workspace-trading.acceptance-harness.scheduler.json`
- example input：`examples/inputs/workspace-trading.acceptance-harness.request.json`
- artifact contract：`docs/workflows/workspace-trading-acceptance-artifact-contract.md`
...
### 6.4 Artifact 输入契约（P1a-3 已冻结）
`collect_and_classify` 当前只认 `workspace-trading` acceptance harness v1 产物，contract 真值单独收口在：
- `docs/workflows/workspace-trading-acceptance-artifact-contract.md`

最小口径如下：
1. `workspace_repo` 必须恒等于 `workspace-trading`
2. `await_terminal.artifacts.artifact_json_path` 必须存在；若 terminal 没给，才允许从 request fallback
3. artifact JSON 必须内嵌：
   - `acceptance_manifest`
   - `acceptance_checklist`
4. `summary.scenario_count` 必须是 `4`
5. `summary.dimensions_covered` 必须恰好覆盖：`etf_basket / stock_basket / oos / regime`
6. `acceptance_manifest.verdict_summary.overall_verdict` 只允许：`PASS / CONDITIONAL / FAIL`
7. checklist 若出现任何 `FAIL` item，整个 workflow 直接记为 `failed`
8. 若未来提供 manifest/checklist sidecar 路径，则必须与 artifact 内嵌内容一致
...
### 7.4 `timeout`
- 业务语义上，`dispatch_acceptance_subagent` 发出后，在 **300 秒** 内未收到 terminal，应视为 `timeout`
- 但 **当前 minimal scheduler 顶层 state 还没有单独的 `timeout` 枚举**，所以 P1a-3 runtime 版先把它收敛成：
...
- runtime definition：`examples/workflows/workspace-trading.acceptance-harness.scheduler.json`
- runtime example input：`examples/inputs/workspace-trading.acceptance-harness.request.json`
- artifact contract：`docs/workflows/workspace-trading-acceptance-artifact-contract.md`
...
- **示例 D**：300 秒无 terminal
  - 业务语义应视为 `timeout`；当前 runtime v1 实际收敛为 `terminal_state=timeout + state=failed`

### 10.4 当前可执行入口（P1a-3）
现在可以直接用 minimal scheduler 跑一遍本地 dry-run：

```bash
python3 scripts/run_minimal_scheduler.py \
  --workflow examples/workflows/workspace-trading.acceptance-harness.scheduler.json \
  --input examples/inputs/workspace-trading.acceptance-harness.request.json \
  --run-dir /tmp/workspace-trading-acceptance-pilot
```

这条命令的定位是：
- **验证 runtime definition 真能推进到终态**
- **验证 `collect_and_classify` 真能消费 trading artifact contract**
- **不代表未来正式 dispatch 一定继续使用 `subagent.local_cli`**；真正接 OpenClaw `sessions_spawn(runtime="subagent")` 仍属于下一步 adapter 升级
...
这份冻结稿完成后，下一步直接进入：
- **E5-T2 剩余部分**：把当前 `subagent.local_cli` 升级成真实 OpenClaw `sessions_spawn(runtime="subagent")` adapter
