    ...
    lines.extend(
        [
            ...
            *_business_callback_contract_lines(dispatch, lifecycle_paths),
            "",
            "## Closeout chain requirements (P0-3 Batch C3)",
            "",
            f"- closeout_chain_required: {canonical_callback.get('closeout_chain_required', True)}",
            f"- closeout_chain_steps: {canonical_callback.get('closeout_chain_steps', ['acceptance_check', 'runtime_closeout', 'git_closeout', 'git_push', 'next_batch_dispatch'])}",
            f"- push_required_before_next_batch: {canonical_callback.get('push_required_before_next_batch', True)}",
            f"- operator_runbook: {canonical_callback.get('operator_runbook', 'examples/trading_roundtable_operator_runbook_v1.md')}",
            "- rule: Each batch MUST complete all closeout chain steps before next batch can start.",
            "- rule: Git push success is a hard prerequisite for next batch dispatch.",
            "",
    ...
    lines.extend(
        [
            ...
            *_business_callback_minimum_contract_lines(dispatch),
            ...
            "",
            "### Callback Output Requirements (P0-3 Batch C3)",
            "",
            "- **MANDATORY**: Subtask MUST output canonical callback payload to `business_callback_output_path`.",
            "- If real business truth is available: write structured callback JSON with `trading_roundtable.packet + trading_roundtable.roundtable`.",
            "- If truth is insufficient: write blocked/degraded callback payload with explicit `blocker` and `missing_business_fields`.",
            "- NEVER output only a generic completion note without callback payload.",
            "",
            "### Closeout Chain (P0-3 Batch C3)",
            "",
            "- After each batch completes, MUST execute: **acceptance → runtime closeout → git closeout → push → next batch**.",
            "- Git push success is a **hard prerequisite** for next batch dispatch.",
            "- See operator runbook: `examples/trading_roundtable_operator_runbook_v1.md` for detailed checklist.",
            "",
        ]
    )
    return "\n".join(lines)
