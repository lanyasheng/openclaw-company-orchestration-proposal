...
class TaskRegistry:
    ...
    def _load_from_file(self, path: Path) -> None:
        ...
        for task_id, task_data in data.get("tasks", {}).items():
            ...
            self.index_by_adapter.setdefault(task.adapter, set()).add(task_id)
            self.index_by_scenario.setdefault(task.scenario, set()).add(task_id)
            self.index_by_status[task.status].add(task_id)
            if task.batch_id:
                self.batches.setdefault(task.batch_id, set()).add(task_id)

        for batch_id, task_ids in data.get("batches", {}).items():
            self.batches[batch_id] = set(task_ids)

    @classmethod
    def load(cls, path: Path) -> "TaskRegistry":
        """从文件加载注册表"""
        registry = cls()
        registry.storage_path = path
