...
def mark_callback_received(task_id: str, result: Dict[str, Any]) -> Dict[str, Any]:
    """标记收到回调"""
    return update_state(task_id, TaskState.CALLBACK_RECEIVED, result=result)


def mark_next_dispatched(task_id: str, next_task_ids: List[str]) -> Dict[str, Any]:
    """标记下一轮任务已派发"""
    return update_state(
        task_id,
...
if __name__ == "__main__":
    ...
    if cmd == "create":
        ...
        try:
            parsed_state = TaskState(new_state)
            result = update_state(task_id, parsed_state)
            print(json.dumps(result, indent=2, ensure_ascii=False))
        except ValueError:
